...
final class Page {
	...
	public function add_submenu(): void {
		add_submenu_page(
			Menu::SLUG_PARENT,
			__( 'CiteWP Settings', 'citewp-ai-search-optimizer' ),
			__( 'Settings', 'citewp-ai-search-optimizer' ),
			'manage_options',
			self::SLUG,
			[ $this, 'render' ]
		);
	...
	public function render(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'CiteWP Settings', 'citewp-ai-search-optimizer' ); ?></h1>
			...
			<?php if ( isset( $_GET['regenerated'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only display flag set by this plugin after safe redirect; no data modification. ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'llms.txt cache cleared. The next request will regenerate from scratch.', 'citewp-ai-search-optimizer' ); ?></p>
				</div>
			<?php endif; ?>
			...
			<?php if ( isset( $_GET['settings-updated'] ) ) : // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Standard WP options-saved flag; no data modification. ?>
				<div class="notice notice-success is-dismissible">
					<p><?php esc_html_e( 'Settings saved.', 'citewp-ai-search-optimizer' ); ?></p>
				</div>
			<?php endif; ?>
			...
			<form method="post" action="options.php">
				...
				<h2><?php esc_html_e( 'Crawler Detection', 'citewp-ai-search-optimizer' ); ?></h2>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable detection', 'citewp-ai-search-optimizer' ); ?></th>
						<td>
							<label>
								...
								<?php esc_html_e( 'Log AI crawler visits to your database', 'citewp-ai-search-optimizer' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="citewp_aiso_log_retention_days"><?php esc_html_e( 'Log retention (days)', 'citewp-ai-search-optimizer' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Free tier: 7 days. Older logs are pruned daily.', 'citewp-ai-search-optimizer' ); ?></p>
						</td>
					</tr>
				</table>
				...
				<h2><?php esc_html_e( 'llms.txt Generation', 'citewp-ai-search-optimizer' ); ?></h2>

				<p>
					<?php esc_html_e( 'Your llms.txt file is served at:', 'citewp-ai-search-optimizer' ); ?>
					<a href="<?php echo esc_url( $llms_short_url ); ?>" target="_blank" rel="noopener"><code><?php echo esc_html( $llms_short_url ); ?></code></a>
					<br />
					<?php esc_html_e( 'Full version (with content bodies):', 'citewp-ai-search-optimizer' ); ?>
					<a href="<?php echo esc_url( $llms_full_url ); ?>" target="_blank" rel="noopener"><code><?php echo esc_html( $llms_full_url ); ?></code></a>
				</p>

				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable llms.txt', 'citewp-ai-search-optimizer' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="<?php echo esc_attr( self::OPTION_LLMS ); ?>[enabled]" value="1" <?php checked( ! empty( $llms['enabled'] ) ); ?> />
								<?php esc_html_e( 'Serve dynamic llms.txt to AI engines', 'citewp-ai-search-optimizer' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="citewp_aiso_min_word_count"><?php esc_html_e( 'Minimum word count for posts', 'citewp-ai-search-optimizer' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Posts shorter than this are skipped (Pages and cornerstone content are always included regardless).', 'citewp-ai-search-optimizer' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="citewp_aiso_recent_days"><?php esc_html_e( 'Include posts from last (days)', 'citewp-ai-search-optimizer' ); ?></label></th>
						<td>
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'Include custom post types', 'citewp-ai-search-optimizer' ); ?></th>
						<td>
			...
			<h2><?php esc_html_e( 'Maintenance', 'citewp-ai-search-optimizer' ); ?></h2>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<p>
					<button type="submit" class="button">
						<?php esc_html_e( 'Regenerate llms.txt now', 'citewp-ai-search-optimizer' ); ?>
					</button>
					<span class="description" style="margin-left:8px;">
						<?php esc_html_e( 'Clears the cache. Next request rebuilds from scratch.', 'citewp-ai-search-optimizer' ); ?>
					</span>
				</p>
			</form>
		</div>
