...
final class Menu {
	...
	public function add_menu(): void {
		...
		add_submenu_page(
			self::SLUG_PARENT,
			__( 'Crawler Logs', 'citewp-ai-search-optimizer' ),
			__( 'Crawler Logs', 'citewp-ai-search-optimizer' ),
			'manage_options',
			self::SLUG_LOGS,
		...
		if ( isset( $submenu[ self::SLUG_PARENT ][0][0] ) ) {
			$submenu[ self::SLUG_PARENT ][0][0] = __( 'Dashboard', 'citewp-ai-search-optimizer' );
		}
	}
	...
	public function render_dashboard(): void {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'CiteWP', 'citewp-ai-search-optimizer' ); ?></h1>
			<p><?php esc_html_e( 'Generative Engine Optimization for WordPress.', 'citewp-ai-search-optimizer' ); ?></p>
			<p>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=' . self::SLUG_LOGS ) ); ?>" class="button button-primary">
					<?php esc_html_e( 'View Crawler Logs', 'citewp-ai-search-optimizer' ); ?>
				</a>
			</p>
		</div>
		<?php
